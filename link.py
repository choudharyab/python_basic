...
class Demo1:
    def __init__(self, master):
        ...
        self.button1 = Tkinter.Button(self.frame, text = 'New Window', width = 25, command = lambda:self.show("Test"))
        self.button1.pack()
        self.frame.pack()
...
class Test:
        ...
        def setupGUI(self):
            label1 = Tkinter.Label(self.root, text='username', width=20, height=3, justify="left", anchor="w",
                                   font=("arial", 10, "bold"))
            label1.grid(row=0, column=0)
            entry1 = Tkinter.Entry(self.root, textvariable=self.username)
            entry1.grid(row=0, column=1)
            label2 = Tkinter.Label(self.root, text='password', width=20, height=3, justify="left", anchor="w",
                                   font=("arial", 10, "bold"))
            label2.grid(row=1, column=0)
            entry2 = Tkinter.Entry(self.root, textvariable=self.password)
            entry2.grid(row=1, column=1)
            b1 = Tkinter.Button(self.root, text='Login', command=self.save, width=10, height=1, justify="left",
        ...
        def save(self):
            ...
            if (self.connection):
                ...
                self.cursor.execute(
                    "INSERT INTO login(username,password) VALUES  ('%s','%s')" % (self.username, self.password))
